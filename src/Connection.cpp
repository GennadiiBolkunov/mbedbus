...
DBusHandlerResult Connection::filterDispatch(DBusConnection* /*conn*/,
    ...
    // Safety barrier: no C++ exception must escape into libdbus C code
    try {
        ...
        MBEDBUS_LOG("Exception in filter dispatch: %s", e.what());
    } catch (...) {
        MBEDBUS_LOG("Unknown exception in filter dispatch");
    }
